...
requests from other Nostr clients, approve or reject them from a live TUI,
and remember per-app decisions so you aren't re-prompted for every action.

On Linux/macOS this starts (or reattaches to) a background daemon that
keeps running after the TUI is closed via "b" or "q" -- reattach any time
with "ncli bunker attach". On Windows the TUI runs directly with no
background support (every other subcommand here requires an already-
running background daemon, which Windows can't provide).`,
...
"ncli bunker" and left running in the background. Fails clearly if none is
running -- this never starts one itself (use "ncli bunker" for that).`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := requireInteractive(cmd); err != nil {
				return err
			}
...
func newConnectCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `With no argument, generates and prints a fresh bunker:// URI (the client-
speaks-first flow) for another Nostr app to connect to. Given a
nostrconnect:// URI (the client-generated, signer-speaks-first flow),
initiates that pairing instead, blocking until the client confirms or the
attempt times out.

--grants <file> pre-authorizes whichever app completes THIS pairing
attempt with a declared set of permissions (see examples/bunker/ for the
"kind: bunker" YAML shape), instead of every method it uses prompting
interactively the first time. Same file works whether written by hand or
generated by an agent script -- ` + "`ncli bunker sessions grants <pubkey>`" + ` shows
what actually landed once paired.`,
